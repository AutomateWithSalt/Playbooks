...
pkg.                     # Module to execute 
pkg.install              # Defines what the module (pkg in this example) should perform
pkg.install vim          # Defines what paramter (vim in this example)
salt '*' pkg.install vim # The compiled line to install vim on all minions

```
## Using Ansible from Salt
```

sudo salt '*' ansible.list        # List all available Ansible Modules
sudo salt '*' ansible.help        # Display help on Ansible standard module
sudo salt '*' ansible.playbook    # Run Ansible Playbooks

####################################################################################
/srv/playbooks                   # Where Ansible Playbooks are Located on the Master

####################################################################################
This will run a playbook on all minions that is called play.yml
####################################################################################

sudo salt '*' ansible.playbook playbook=/srv/playbooks/play.yml
